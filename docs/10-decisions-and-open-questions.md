...
| D91 | **A check form travels as a file, and lands as a draft** | Building a twenty-field form is an afternoon, and building it again on a staging box or after another house improved theirs is the same afternoon. The export is the name, the description and the field set, and nothing else: no template id, no version number, no publication history, no records. It always imports as a new, unpublished draft, never as an update to an existing form, because publishing is irreversible and entries are bound to a published version, and neither of those may be reachable by dropping a file on a screen. A colliding name is marked "(imported)" rather than overwriting, so re-importing the same file while setting an installation up is untidy instead of destructive. That marking is honest and is a terrible thing to leave on a form staff read for years, so a form can be renamed in place: the name and the description are the two things about a form that are not versioned, and changing them touches no record. A rename onto the name of another **active** form is refused, because two forms with one name is a schedule pointed at the wrong one, which is exactly what the import marking exists to avoid. The whole file lands or none of it does, and a `version` the code does not recognise is refused rather than read with the unknown parts dropped |
| D92 | **A medication sign-off records the amount actually given, as free text** | The chart says "500 mg" or "1 to 2 tablets"; what went in was sometimes half a tablet because that is what was left, or 7.5 mL drawn up rather than 10. That gap is the thing worth recording and there was nowhere to put it. Free text like `medications.dose`, for the same reason: **nothing converts, totals or compares it**, because software that does arithmetic on doses is software that can get a dose wrong (CLAUDE.md). Optional, and blank means the record does not say, never "the charted amount". Refused into an amount is refused: an amount cannot sit on a sign-off that says nothing was given, checked by the shared rule so the phone refuses it with no signal |
| D93 | **The timeline reads newest day first, and each day forwards** | Two questions, two directions. Which day am I looking at is answered by today being at the top, so the screen opens on the shift being handed over. What happened on that day is answered by reading down the page from the morning, which is how a day is told, and how the self-access "my day" screen already read it while the staff timeline read the opposite way. The check rows now name who filled the form in, carried on the window itself so a row can say it without fetching every entry behind it |
| D94 | **An admin ticks which check forms apply to each participant** | Recording a check on demand offered every published form in the org. For a team running a bowel chart, a seizure record and two observation forms, a worker recording something for somebody who only ever needs one of them had three chances to file it on the wrong form, and the list grew every time the org wrote another. `participant_check_forms` is the tick list and the picker reads it. **It is not a permission**: everything on that screen is already inside a record the caller is in scope for, and ticking a form decides what is offered, nothing else. A form on the participant's **active schedule is included without being ticked**, and cannot be unticked, because the schedule already said the form is theirs and unticking it would not stop the windows arriving. A draft form can be ticked and starts appearing the moment it is published. The migration ticks every active form for every participant, because an empty table reads as "nobody has any forms" and would take on-demand recording away from the whole org until an admin had been through every person |
| D95 | **The diary is the day book: an entry can be dated ahead, and it opens on a month grid** | The diary was modelled as a record of what had already happened, and `occurredAtProblem` refused anything more than a few minutes in the future. That made the thing the paper diary was actually for impossible: staff write down what a participant has coming up. Notes on how something went live in the team's separate notes app, so this is not that. An entry may now be dated up to **730 days ahead**, still 90 days back, and `recordedAt` is unchanged alongside it so when it was written is never disguised by when it is for. The tab opens on the current month with a marker on every day that has something on, and the day below it, so opening the diary answers "what has Alice got on today" without anybody typing a query. Search stays as its own view, because "when was the last dental appointment" is a question about the whole record rather than about a day. The far-future bound is a typo guard, not a policy: 2029 is a real appointment, 2926 is a slipped keystroke |
| D96 | **An admin can add a note to a recorded check, and the reading never moves** | A check form records what was observed and nothing else, which is the point. But somebody reading a completed observation afterwards often has to say something about it: why the output was low, what was done, what was discussed. The only writable surface on a recorded check was the values themselves, so the choice was editing a reading to explain it, which destroys the reading, or having nowhere to put it. `check_entry_notes` sits beside the entry: **append-only, enforced by the grant** like a medication sign-off and the audit log, because a note gets acted on and one that could be reworded afterwards is not evidence of anything. Encrypted, like every note in Vigilo. Adding one writes no revision and does not touch `edit_count`: a note explains a record, it does not alter one. **Admin writes, every staff role reads**, deliberately a shorter list than `canEditOthersEntries` and deliberately not nothing, because an explanation only its author can see explains nothing. It travels with the check onto the daily report and the PDF, and never onto a participant's own record: the self-access DTOs are built field by field (D71) and `/check-entries` is not on the allow-list (D70). Server-only, like the edit history, so a device with no signal shows none rather than queueing one |
| D97 | **Ending a schedule takes every window still open with it** | Changing a schedule's segments preserves the window in progress on purpose: an admin moving the times should not pull the grid out from under a worker partway through a check. Ending the schedule is the opposite instruction and was running the same code, so the window that happened to be open stayed on the screen asking for a form nobody wants, and the closer marked it missed within the hour. That is the same complaint D85 answers for windows laid in the past: a missed check says staff did not do something they were asked to do. So `removeOpenWindows` clears everything that has not closed, in progress or not, with a tombstone so devices holding one are told. A window that **already holds an entry is kept**, complete or partial: the values are a record somebody made, deleting the window would cascade them away, and nothing here hard-deletes a clinical record to tidy a schedule up. Closed windows are history and are never touched |
| D98 | **Source-available under PolyForm Noncommercial 1.0.0, commercial use by written permission only** | The repository is public and carried no licence at all, which under copyright means nobody could lawfully use any of it, including the people it was written for. The intent is the opposite of a reservation: read it, run it, fork it, and self-host it in a charity, a school, a public health service or a government team, whatever their funding. What is reserved is **making money from it**. PolyForm Noncommercial rather than a bespoke licence, because a non-standard licence is a lawyer's afternoon for every organisation that considers it, and rather than CC BY-NC, which is not written for software and does not address patents. `PolyForm-Noncommercial-1.0.0` is a real SPDX identifier, so tooling and the image labels can state it. **A commercial licence is granted case by case, in writing, to a named entity, and by nothing else**: not a README line, not an issue comment, not silence. The distinction that matters is the entity and not the fee, so a for-profit provider running it internally for its own staff and charging nobody still needs permission, and that is on purpose. Contributions are accepted on the basis that Fenici may licence the result commercially, otherwise the licensor cannot grant what it says it grants |
| D99 | **Three ways in beside the password: a passkey, a device PIN and the device's fingerprint** | Email and password on a phone, at a front door, in the rain, before every shift, is the friction that gets a password written on the inside of a folder. So: a **passkey** is a real credential the authenticator holds, registered with the server and verified against it, and it travels, because a passkey in a phone's keychain signs the same person in on the next phone with nobody reissuing anything. It **stands in for the second factor too**, because the ceremony only completes after the device has verified the person, and the server checks the verified flag rather than trusting that it asked for it. **Quick sign-in** is the other kind of thing entirely and is deliberately not a factor (doc 01 §10): a secret the server issued to a session that already existed, sealed in the device's own secure store behind its fingerprint or a six-digit PIN, redeemed for the session the account already earned. It is per device, it expires unused after sixty days, a wrong secret retires it outright rather than counting attempts, and changing the password or signing out kills it. **Biometric quick sign-in is offered only on an installed app on a handheld**: a fingerprint that unlocks a browser tab is a credential sitting in a tab nobody closes, and a desktop reader is usually a shared office machine. The PIN is offered wherever the fingerprint is not, so this can never be the reason somebody cannot get in. **The password never goes away** and none of these can be set up by an admin: there is no email in this system, so an account whose only credential is on a lost phone is an account nobody can hand back |
